...
class TableWidget(QWidget):
    ...
    def initUI(self):
        ...
        self.label_comboBox.setToolTip(
            "Select the sample category used to draw the histogram."
        )
    ...
    def init_data(self, header, data):
        ...
        for i in range(0, self.data.shape[0], self.row):
                ...
                i + self.row
                if i + self.row < self.data.shape[0]
        ...
        try:
            if not self.data is None:
                if "home" == signal[0] and signal[1] != "":
                    ...
                    page = (
                        int(signal[1]) + 1
                        if int(signal[1]) + 1 <= int(self.final_page)
                        else int(self.final_page)
                    )
                    self.curPage.setText(str(page))
                    self.display_table(str(page))
    ...
    def display_histogram(self):
        if self.selected_column is None:
            QMessageBox.critical(
                self,
                "Error",
                "Please select a column (except col 0)).",
                QMessageBox.Ok | QMessageBox.No,
                QMessageBox.Ok,
            )
            ...
            data = np.hstack(
                (
                    self.data[1:, 1].reshape((-1, 1)),
                    self.data[1:, self.selected_column].reshape((-1, 1)),
                )
            ).astype(float)
        ...
        if self.selected_column is None:
            ...
            data = np.hstack(
                (
                    ...
                    self.data[:, self.selected_column].reshape((-1, 1)),
                )
            ).astype(float)
            self.hist = HistogramWidget()
            self.hist.init_data(self.header[self.selected_column], data)
