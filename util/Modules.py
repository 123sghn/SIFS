# A GUI class for plotting ROC curves, PRC curves, scatter plots, and kernel density plots using user-defined data.
...
class PlotCurve(QMainWindow):
    ...
    def initUI(self):
        ...
        self.setWindowTitle("SIFS Plot curve")
        self.resize(600, 600)
    ...
    def importData(self):
        auc, dot, color, lineWidth, lineStyle, prefix, raw_data, ok = (
            InputDialog.QPlotInput.getValues(self.curve)
        )
        if not auc is None and not prefix is None and ok:
            self.raw_data[prefix] = raw_data
            if self.curve == "ROC":
                self.data.append(
                    ["%s AUC = %s" % (prefix, auc), dot, lineWidth, lineStyle, color]
                )
            else:
                self.data.append(
                    ["%s AUPRC = %s" % (prefix, auc), dot, lineWidth, lineStyle, color]
                )
    ...
    def compareCurves(self):
        if len(self.raw_data) >= 2:
            ...
            if ok:
                self.subWin = PlotWidgets.BootstrapTestWidget(
                    self.raw_data, bootstrap_n, self.curve
                )
            ...
            QMessageBox.critical(
                self,
                "Error",
                "Two or more curve could be compared!",
    ...
    def importData(self):
        try:
            ...
            if ok:
                ...
                self.plotGraph = PlotWidgets.ClusteringDiagramMatplotlib()
                self.plotGraph.init_data(
                    "Scatter Plot", new_data, data.columns[1], data.columns[2]
                )
                self.plotLayout.addWidget(self.plotGraph)
...
class Hist(QMainWindow):
    ...
    def initUI(self):
        ...
        self.setWindowTitle("SIFS Histogram and Kernel density plot")
        self.resize(600, 600)
        self.setWindowIcon(QIcon("images/logo.ico"))
        plotWidget = QWidget()
        self.plotLayout = QVBoxLayout(plotWidget)
