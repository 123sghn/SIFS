# A GUI class for plotting ROC curves, PRC curves, scatter plots, and kernel density plots.
#!/usr/bin/env python
...
class ClusteringDiagramMatplotlib(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle('SIFS Scatter plot')
        self.setWindowIcon(QIcon('images/logo.ico'))
        self.resize(800, 600)
...
class LossWidget(QWidget):
    def __init__(self):
        super(LossWidget, self).__init__()
        self.fontdict = {
            'family': 'Arial',
            'size': 16,
            'color': '#282828',
    ...
    def initUI(self):
        self.setWindowTitle('SIFS Loss Curve')
        self.setWindowIcon(QIcon('images/logo.ico'))
        self.resize(800, 600)
        layout = QVBoxLayout(self)
        self.figureCanvas = MyFigureCanvas()
    ...
    def init_data(self, title, loss_data):        
        try:
            self.fig = self.figureCanvas.figure.add_subplot(111)
            self.fig.cla()  
            self.fig.plot(loss_data, label='Loss Curve', color='blue')  
            self.fig.set_title(title, fontdict=self.fontdict)  
            self.fig.set_xlabel('Epoch', fontdict=self.fontdict)  
            self.fig.set_ylabel('Loss', fontdict=self.fontdict)  
            self.fig.legend()  
...
class HistogramWidget(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle('SIFS Histogram')
        self.setWindowIcon(QIcon('images/logo.ico'))
        self.resize(800, 600)
        layout = QVBoxLayout(self)
        self.figureCanvas = MyFigureCanvas()
...
class CurveWidget(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle('SIFS Curve')
        self.setWindowIcon(QIcon('images/logo.ico'))
        self.resize(800, 600)
        layout = QVBoxLayout(self)
        self.figureCanvas = MyFigureCanvas()
    ...
    def __draw_figure__(self):
        ...
        if not self.fold_data is None:
            for i, item in enumerate(self.fold_data):
                self.fig.plot(np.array(item[1][self.x]), np.array(item[1][self.y]), label=item[0], color=self.colorlist[i%len(self.colorlist)], lw=2, alpha=1.0)
        if not self.mean_data is None:
            self.fig.plot(np.array(self.mean_data[1][self.x]), np.array(self.mean_data[1][self.y]), label=self.mean_data[0], lw=2,
                          alpha=0.8, color='b')
        if not self.ind_data is None:
            self.fig.plot(np.array(self.ind_data[1][self.x]), np.array(self.ind_data[1][self.y]), label=self.ind_data[0], lw=2, alpha=0.5,
                          color='r')
        if self.type == 0:
            self.fig.plot(np.array([0, 1]), np.array([0, 1]), label='Random', lw=2, alpha=0.5, linestyle='dashed', color='#696969')
        self.fig.legend()
        labels = self.fig.get_xticklabels() + self.fig.get_yticklabels()
        [label.set_color('#282828') for label in labels]
        [label.set_fontname('Arial') for label in labels]
...
class HeatmapWidget(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle('SIFS Heatmap')
        self.setWindowIcon(QIcon('images/logo.ico'))
        self.resize(800, 600)
        layout = QVBoxLayout(self)
        self.figureCanvas = MyFigureCanvas()
...
class BootstrapTestWidget(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle('SIFS Bootstrap test')
        self.setWindowIcon(QIcon('images/logo.ico'))
        self.resize(800, 600)
        layout = QVBoxLayout(self)
        self.figureCanvas = MyFigureCanvas()
