...
class Metrics(object):
    ...
    def calculateBinaryTaskMetrics(self):
        ...
        self.f1 = (
            round(2 * tp / (2 * tp + fp + fn), 4) if (2 * tp + fp + fn) != 0 else 0
        )
        # roc
        fpr, tpr, _ = roc_curve(self.labels, self.scores, pos_label=self.pos_label)
        self.auc = round(auc(fpr, tpr), 4)
        self.aucDot = pd.DataFrame(
            np.hstack((fpr.reshape((-1, 1)), tpr.reshape((-1, 1)))),
            columns=["fpr", "tpr"],
        )
        # prc
        precision, recall, _ = precision_recall_curve(
            self.labels, self.scores, pos_label=self.pos_label
        )
        self.prc = round(auc(recall, precision), 4)
        self.prcDot = pd.DataFrame(
            np.hstack((recall.reshape((-1, 1)), precision.reshape((-1, 1)))),
    ...
    def calculateMultiTaskMetrics(self):
        ...
        self.accuracy = round(
            len(result[result == self.labels]) / len(self.labels) * 100, 2
        )
    ...
    def getBinaryTaskMetrics(scores, labels, pos_label=None, threshold=0.5):
        ...
        return (
            ...
            evaluationMetrics.auc,
            evaluationMetrics.prc,
        ), True
